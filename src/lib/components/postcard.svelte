  ...
  <div class="flex flex-row h-full">
      ...
      <div class="w-full">
        <div class="flex flex-row">
          ...
          <h4 class="font-bold">&nbsp;[&nbsp;{post.gamertag}&nbsp;]&nbsp;<span class="text-accent-500">{post.title}</span></h4>
        </div>
    ...
    <div class="w-1/6 border-l-2 h-full">
        <p>
          ...
          {:else if postedWhen(post) <= 5}
            <span class="text-sm text-accent-500"><span class="animate-ping">&#x1F7E2;</span> Posted {postedWhen(post)}m ago</span>
          {:else}
            <span class="text-sm">&nbsp;Posted {postedWhen(post)}m ago</span>
          {/if}</p>
        <hr />
        {#if post.username === $page?.data.session?.user.user_metadata.username}
          <button type="button" class="hover:text-accent-500 mt-2 pl-1 text-sm"  on:click={() => { window.location.href = "/editpost?id=" + post.id }}>&#183; Update post</button>
          <button type="button" class="hover:text-accent-500 pl-1 text-sm"  on:click={() => { window.location.href = "/myposts" }}>&#183; Manage posts</button>
        {:else}
