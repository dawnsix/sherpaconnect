  ...
  <div class="flex flex-row h-full">
      ...
      <div class="w-full">
        <div class="flex flex-row">
          <div class="mt-2.5">
            {#if post.platform === "ps"}
            <i><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#9AF9A2" class="bi bi-playstation" viewBox="0 0 16 16">
            ...
            <i><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#9AF9A2" class="bi bi-xbox" viewBox="0 0 16 16">
            ...
            {/if}
            {#if post.platform === "pc"}
            <i><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#9AF9A2" class="bi bi-pc-display-horizontal" viewBox="0 0 16 16">
              <path d="M1.5 0A1.5 1.5 0 0 0 0 1.5v7A1.5 1.5 0 0 0 1.5 10H6v1H1a1 1 0 0 0-1 1v3a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-3a1 1 0 0 0-1-1h-5v-1h4.5A1.5 1.5 0 0 0 16 8.5v-7A1.5 1.5 0 0 0 14.5 0h-13Zm0 1h13a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-.5.5h-13a.5.5 0 0 1-.5-.5v-7a.5.5 0 0 1 .5-.5ZM12 12.5a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0Zm2 0a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0ZM1.5 12h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1 0-1ZM1 14.25a.25.25 0 0 1 .25-.25h5.5a.25.25 0 1 1 0 .5h-5.5a.25.25 0 0 1-.25-.25Z"/>
            </svg></i>
            {/if}
          </div>

          <h4 class="font-bold">&nbsp;[&nbsp;{post.gamertag}&nbsp;]&nbsp;<span class="text-accent-200 dark:text-accent-500">{post.title}</span></h4>
        </div>
          
        <hr/>
        <p><span class="font-bold">Category: </span><span class="dark:text-accent-400 text-accent-200">{post.type}</span></p>
        <p><span class="font-bold">Description: </span></p>
        <p class="pl-5"><span class="dark:text-accent-400 text-accent-200">{post.desc}</span></p>
    </div>
    ...
    <div class="w-1/6 border-l-2 h-full">
        ...
        {#if post.username === $page?.data.session?.user.user_metadata.username}
          <button type="button" class="hover:text-accent-400 mt-2 pl-1 text-sm"  on:click={() => { window.location.href = "/editpost?id=" + post.id }}>&#183; Update post</button>
          <button type="button" class="hover:text-accent-400 pl-1 text-sm"  on:click={() => { window.location.href = "/myposts" }}>&#183; Manage posts</button>
        {:else}
        ...
        <div class="flex flex-col pt-5 mr-0.5">
          <span>
            <h4 class="font-bold float-right mr-1">
              <span class="text-accent-200 dark:text-accent-500">&nbsp;Price: 
              </span>${post.price}
            </h4>
          </span>
        </div>
